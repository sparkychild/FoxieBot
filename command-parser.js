'use strict';
class Context {
    constructor(target, user, room, command) {
        this.user = user;
        this.room = room;
    ...
    parse(post) {
        return commandParser(post.replace(/^\//i, this.room ? this.room.commandCharacter[0] : Config.defaultCharacter[0]), this.user, this.room, true);
    }
}

exports.commandParser = function(message, user, room, bypassMonitor) {
    ...
    if (Commands[command]) {
        ...
        let context = new Context(target, user, room, command);
        let success;
        try {
